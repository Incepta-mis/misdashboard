...
class Dcr_Dwiu_Rep_Controller extends Controller
{
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////DCR DOCTOR WISE ITEM UTILIZATION REPORT/////////////////////////////////
    /////////////////////////////////////////////////////////////////////////////////////////////////////

    protected function dwiu_view(){
        ...
        if (Auth::user()->desig === 'RM') {
            ...
            $array = ['pgrp' => $p_group, 'am_terr' => $am_terr, 'terr' => $terr_id];
            ...
            $array = ['pgrp' => $p_group, 'rm_terr' => $rm_terr, 'am_terr' => $am_terr, 'terr' => $terr_id];
        ...
        return view('dcr_reports_layout.v_doctor_wise_item_utilization_v2')
            ->with($array);

    }


    protected function dwui_data(Request $request){
        if($request->ajax()){
            $responses = DB::select("select PRODUCT_GROUP, TERR_ID, 
                                   EMPLOYEE_ID, EMPLOYEE_NAME, DOCTOR_ID, 
                                   DOCTOR_NAME, ITEM_ID, ITEM_NAME,sum(QTY_UTILIZED) QTY_UTILIZED
                                    from (
                                   SELECT PRODUCT_GROUP, TERR_ID, 
                                   EMPLOYEE_ID, EMPLOYEE_NAME, DOCTOR_ID, 
                                   DOCTOR_NAME, ITEM_ID, ITEM_NAME, 
                                   sum(QTY_UTILIZED) QTY_UTILIZED,substr(terr_id,1,instr(terr_id,'-',1,1))||trunc(substr(terr_id,instr(terr_id,'-',-1,1)+1),-1) AM_TERR_ID,
    ...
    protected function dwui_terr_doc_list(Request $request){
        if($request->ajax()){
            ...
            if($request['type'] == 2)
            {
                ...
                $resp_doc_list = DB::select("select distinct doctor_name,doctor_id
                                         ...
                                         where PRODUCT_GROUP = decode('$request->pgrp','All',PRODUCT_GROUP,'$request->pgrp')
                                         and terr_id = decode('$request->tid','All',terr_id,'$request->tid')
    ...
    protected function dwiu_terr_list(Request $request){
