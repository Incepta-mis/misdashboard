...
class Dcr_Dvsr_Rep_Controller extends Controller
{
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////DCR DOCTOR VISIT SUMMARY REPORT///////////////////////////////////
    /////////////////////////////////////////////////////////////////////////////////////////////////////

    protected function dvsr_view()
    {
        ...
        if (Auth::user()->desig === 'RM') {
            ...
            $array = ['pgrp' => $p_group, 'am_terr' => $am_terr, 'terr_id' => $terr_id, 'dcr_mnth' => $dcr_month];
            ...
            $array = ['pgrp' => $p_group, 'rm_terr' => $rm_terr, 'am_terr' => $am_terr, 'terr_id' => $terr_id, 'dcr_mnth' => $dcr_month];
            ...
            $terr_id = DB::select("Select terr_id from  MIS.DCR_DOCTOR_VISIT_SUMMARY 
        ...
        return view('dcr_reports_layout.v_doctor_visit_summary_v2')
            ->with($array);

    }

    protected function dvsr_data(Request $request)
    {
        if ($request->ajax()) {

            $resp_data = DB::select("select PRODUCT_GROUP, TERR_ID,EMPLOYEE_ID, EMPLOYEE_NAME,NO_OF_DOCTOR_VISIT,NO_OF_CALL
                                    from(
                                    SELECT PRODUCT_GROUP, TERR_ID,EMPLOYEE_ID, EMPLOYEE_NAME, nvl(NO_OF_DOCTOR_VISIT,0) NO_OF_DOCTOR_VISIT, 
                                           nvl(NO_OF_CALL,0) NO_OF_CALL,substr(terr_id,1,instr(terr_id,'-',1,1))||trunc(substr(terr_id,instr(terr_id,'-',-1,1)+1),-1) AM_TERR_ID,
                                    ...
                                    and dcr_date = to_date('$request->dat1','MON-YY')");
    ...
    protected function terr_list(Request $request)
    {
        if ($request->ajax()) {
