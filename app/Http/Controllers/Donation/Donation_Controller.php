...
class Donation_Controller extends Controller
{
    public function index()
    {
        ...
        return view('donation.donation_requisition')->with( ['depo' => $depo,'dtm' => $dtm, 'dpm' => $dpm,'dbt' => $dbt,'freq' => $freq, 'docid' => $docid,'month_name' => $month_name]);
    ...
    public function brand_region_post(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            (select distinct dscc.cost_center_id cost_center_id,dscc.sub_cost_center_id sub_cost_center_id,dscc.sub_cost_center_name sub_cost_center_name,dcc.cost_center_group
                           ...
                           and mpo_emp_id = '$uid' and p_group <> 'GENERAL'
                           union all
                           ...
                           where to_char(ht.emp_month,'DD-MON-YY')= to_date(trunc(sysdate,'MONTH'),'DD-MON-RR')
                           and mpo_emp_id = '$uid' and p_group = 'GENERAL') tgr
...
//        if ($request->ajax()) {
        ...
        if($rq[0]->req_npm == '0'){
            ...
            $dcn = DB::insert(" insert into mis.donation_requisition (                
                ...
                values 
                (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'YES',?,?,?,?,?)",[$request->inf_ori,$request->dpid,$request->dpname,$reqid[0]->req_id,$systime[0]->sysdate,$uid,$uname,$tid,$request->don_type,$request->group,$request->beneficiary,
