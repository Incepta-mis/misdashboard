...
class StabilityMonitoringDataPDBatch01Controller extends Controller
{
    public function index()
    {
        ...
        order by 1 asc');
        return view('ssm_views.reports.result.stabilitymonitoringdataforpdbatch_01', compact('pname', 'batch_number', 'test_parameters'));
    }
...
and average_result_time in ('INITIAL','1M')
and sd.pname = ?
and sd.batch_number = ?
union all
select sd.pname,sd.batch_number,kept_on_date,mode_of_pack,p_size,test_parameters,accept_criteria,
...
and average_result_time in ('1M')
and sd.pname = ?
and sd.batch_number = ?
))pivot (max(average_result) for row_no in (1,2,3))
...
and sd.pname = ?
and sd.batch_number = ?
))pivot (max(average_result) for row_no in (1 as a,2 as b,3 as c))
where ? = case when ? = 'ALL' then all_data else test_parameters end
order by tp_no",
...
//            dd ($data);
            ...
            \Excel::create('Summary', function ($excel) use ($data) {
                ...
                $excel->sheet('Summary', function ($sheet) use ($data) {
                    $sheet->loadView('ssm_views.reports.result.stabilitymonitoringdataforpdbatch_01excellayout', $data);
...
order by tp_no",
            ...
            $pdf = \SPDF::loadView('ssm_views.reports.result.stabilitymonitoringdataforpdbatch_01pdflayout', ['pdata1' => $data_pr,'pname'=>$parameters['pname'],'batch_number'=>$parameters['batch_number']
                ,'KOD'=>$data_pr[0]->kept_on_date,'PS'=>$data_pr[0]->p_size,'MOP'=>$data_pr[0]->mode_of_pack]);
