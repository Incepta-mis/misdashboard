...
class StabilityMonitoringDataECountry04Controller extends Controller
{
    public function index()
    {
        ...
        order by 1 asc');
        return view('ssm_views.reports.result.stabilitymonitoringdataecountry04',compact('pname','batch_number','export_country','test_parameters'));
    }
...
from(
select sd.pname,country_name,sd.batch_number,kept_on_date,mode_of_pack,p_size,test_parameters,accept_criteria,
       average_result,average_result_time,tp_no,sl,substr(chamber_stor_cond,1,2) csc 
from mis_fac.average_result_data ard,(select distinct pname,export_country country_name,batch_number,kept_on_date,mode_of_pack,p_size
...
",
            ...
            [$request->pname,$request->batch_number,$request->export_country,$request->test_parameters,$request->test_parameters]);
...
and sd.batch_number = ?
and sd.Country_Name = ?
))pivot (max(average_result) for row_no in (11 as a,2 as b,3 as c,4 as d,5 as e,6 as f,7 as g))
where ? = case when ? = 'ALL' then all_data else test_parameters end
order by test_parameters",
                [$parameters['pname'], $parameters['batch_number'],$parameters['export_country'],
...
//            dd ($data);
            ...
            \Excel::create('Summary', function ($excel) use ($data) {
                ...
                $excel->sheet('Summary', function ($sheet) use ($data) {
                    $sheet->loadView('ssm_views.reports.result.stabilitymonitoringdataecountry04excellayout', $data);
...
where ? = case when ? = 'ALL' then all_data else test_parameters end
order by tp_no",
                [$parameters['pname'], $parameters['batch_number'],$parameters['export_country'], $parameters['test_parameters'],$parameters['test_parameters']]);


//dd($data_pr);
            $pdf = \SPDF::loadView('ssm_views.reports.result.stabilitymonitoringdataecountry04pdflayout', ['pdata1' => $data_pr,'pname'=>$parameters['pname'],'batch_number'=>$parameters['batch_number']
                ,'KOD'=>isset($data_pr[0]->kept_on_date) ? $data_pr[0]->kept_on_date : null,'PS'=>isset($data_pr[0]->p_size) ? $data_pr[0]->p_size : null,'MOP'=>isset($data_pr[0]->mode_of_pack) ? $data_pr[0]->mode_of_pack : null]);
