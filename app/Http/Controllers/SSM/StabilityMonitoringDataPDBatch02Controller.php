...
class StabilityMonitoringDataPDBatch02Controller extends Controller
{
    public function index()
    {
        ...
        order by 1 asc');
        return view('ssm_views.reports.result.stabilitymonitoringdataforpdbatch_02', compact('pname', 'batch_number', 'test_parameters'));
    }
    ...
    public function getvalues(Request $request)
    {
        ...
        return response()->json(['batch_number' => $batch_number, 'test_parameters' => $test_parameters]);
    }
...
from
(select 'ALL' all_data,pname,batch_number,kept_on_date,mode_of_pack,p_size,test_parameters,accept_criteria,average_result,tp_no,
        row_number() over (partition by tp_no order by tp_no,csc,sl) as row_no
from(
select sd.pname,sd.batch_number,kept_on_date,mode_of_pack,p_size,test_parameters,accept_criteria,
       average_result,average_result_time,tp_no,sl,substr(chamber_stor_cond,1,2) csc 
from mis_fac.average_result_data ard,(select distinct pname,batch_number,kept_on_date,mode_of_pack,p_size
...
and average_result_time in ('INITIAL','1M','2M')
and sd.pname = ?
and sd.batch_number = ?
union all
select sd.pname,sd.batch_number,kept_on_date,mode_of_pack,p_size,test_parameters,accept_criteria,
...
and average_result_time in ('1M','2M')
and sd.pname = ?
and sd.batch_number = ?
))pivot (max(average_result) for row_no in (1 ,2 ,3 ,4 ,5))
where ? = case when ? = 'ALL' then all_data else test_parameters end
order by tp_no",
            [$request->pname, $request->batch_number,$request->pname, $request->batch_number, $request->test_parameters, $request->test_parameters]);
...
and sd.pname = ?
and sd.batch_number = ?
))pivot (max(average_result) for row_no in (1 as a,2 as b,3 as c,4 as d,5 as e))
where ? = case when ? = 'ALL' then all_data else test_parameters end
order by tp_no",
                [$parameters['pname'], $parameters['batch_number'],$parameters['pname'], $parameters['batch_number'],$parameters['test_parameters'], $parameters['test_parameters']]);


            $data = ['pdata1' => $data_pr,'pname'=>$parameters['pname'],'batch_number'=>$parameters['batch_number']
                ,'KOD'=>$data_pr[0]->kept_on_date,'PS'=>$data_pr[0]->p_size,'MOP'=>$data_pr[0]->mode_of_pack];
//            dd ($data);

            \Excel::create('Summary', function ($excel) use ($data) {

                $excel->sheet('Summary', function ($sheet) use ($data) {
                    $sheet->loadView('ssm_views.reports.result.stabilitymonitoringdataforpdbatch_02excellayout', $data);
...
order by tp_no",
            ...
            $pdf = \SPDF::loadView('ssm_views.reports.result.stabilitymonitoringdataforpdbatch_02pdflayout', ['pdata1' => $data_pr,'pname'=>$parameters['pname'],'batch_number'=>$parameters['batch_number']
                ,'KOD'=>$data_pr[0]->kept_on_date,'PS'=>$data_pr[0]->p_size,'MOP'=>$data_pr[0]->mode_of_pack]);
