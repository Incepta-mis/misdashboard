...
class StabilityMonitoringData24M1Controller extends Controller
{
    public function index()
    {
        ...
        order by 1 asc');
        return view('ssm_views.reports.result.stabilitymonitoringdata24m1',compact('pname','batch_number','test_parameters'));
    }
...
(select 'ALL' all_data,pname,batch_number,kept_on_date,mode_of_pack,p_size,test_parameters,accept_criteria,average_result,tp_no,
        row_number() over (partition by tp_no,generic_name,accept_criteria order by tp_no,csc,sl) as row_no
from(
select sd.pname,generic_name,sd.batch_number,kept_on_date,mode_of_pack,p_size,test_parameters,accept_criteria,
...
and sd.pname = ?
and sd.batch_number = ?
))pivot (max(average_result) for row_no in (1,2,3,4,5,6,7,8,9))
...
where ? = case when ? = 'ALL' then all_data else test_parameters end
order by tp_no",
                [$parameters['pname'], $parameters['batch_number'],$parameters['pname'], $parameters['batch_number'],
...
//            dd ($data);
            ...
            \Excel::create('Summary', function ($excel) use ($data) {
                ...
                $excel->sheet('Summary', function ($sheet) use ($data) {
                    $sheet->loadView('ssm_views.reports.result.stabilitymonitoringdata24m1excellayout', $data);
...
order by tp_no",
            ...
            $pdf = \SPDF::loadView('ssm_views.reports.result.stabilitymonitoringdata24m1pdflayout', ['pdata1' => $data_pr,'pname'=>$parameters['pname'],'batch_number'=>$parameters['batch_number']
                ,'KOD'=>$data_pr[0]->kept_on_date,'PS'=>$data_pr[0]->p_size,'MOP'=>$data_pr[0]->mode_of_pack]);
