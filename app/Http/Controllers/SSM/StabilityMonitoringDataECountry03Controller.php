...
class StabilityMonitoringDataECountry03Controller extends Controller
{
    public function index()
    {
        ...
        order by 1 asc');
        return view('ssm_views.reports.result.stabilitymonitoringdataecountry03',compact('pname','batch_number','export_country','test_parameters'));
    }
...
//            dd ($data);
            ...
            \Excel::create('Summary', function ($excel) use ($data) {
                ...
                $excel->sheet('Summary', function ($sheet) use ($data) {
                    $sheet->loadView('ssm_views.reports.result.stabilitymonitoringdataecountry03excellayout', $data);
...
//dd($data_pr);
            $pdf = \SPDF::loadView('ssm_views.reports.result.stabilitymonitoringdataecountry03pdflayout', ['pdata1' => $data_pr,'pname'=>$parameters['pname'],'batch_number'=>$parameters['batch_number']
                ,'KOD'=>isset($data_pr[0]->kept_on_date) ? $data_pr[0]->kept_on_date : null,'PS'=>isset($data_pr[0]->p_size) ? $data_pr[0]->p_size : null,'MOP'=>isset($data_pr[0]->mode_of_pack) ? $data_pr[0]->mode_of_pack : null]);
