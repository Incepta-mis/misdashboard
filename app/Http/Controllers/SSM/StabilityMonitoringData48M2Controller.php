...
class StabilityMonitoringData48M2Controller extends Controller
{
    public function index()
    {
        ...
        order by 1 asc');
        return view('ssm_views.reports.result.stabilitymonitoringdata48M2', compact('pname', 'batch_number', 'test_parameters'));
    }
...
and average_result_time in ('INITIAL','3M','6M','9M','12M','18M','24M','36M','48M')
and sd.pname = ?
and sd.batch_number = ?
...
//            dd ($data);
            ...
            \Excel::create('Summary', function ($excel) use ($data) {
                ...
                $excel->sheet('Summary', function ($sheet) use ($data) {
                    $sheet->loadView('ssm_views.reports.result.stabilitymonitoringdata48M2excellayout', $data);
...
order by tp_no",
            ...
            $pdf = \SPDF::loadView('ssm_views.reports.result.stabilitymonitoringdata48M2pdflayout', ['pdata1' => $data_pr,'pname'=>$parameters['pname'],'batch_number'=>$parameters['batch_number']
                ,'KOD'=>$data_pr[0]->kept_on_date,'PS'=>$data_pr[0]->p_size,'MOP'=>$data_pr[0]->mode_of_pack]);
