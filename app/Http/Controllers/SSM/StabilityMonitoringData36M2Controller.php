...
class StabilityMonitoringData36M2Controller extends Controller
{
    public function index()
    {
        ...
        order by 1 asc');
        return view('ssm_views.reports.result.stabilitymonitoringdata36M2', compact('pname', 'batch_number', 'test_parameters'));
    }
...
(select 'ALL' all_data,pname,batch_number,kept_on_date,mode_of_pack,p_size,test_parameters,accept_criteria,average_result,tp_no,
        row_number() over (partition by tp_no,accept_criteria order by tp_no,csc,sl) as row_no
from(
...
and average_result_time in ('INITIAL','3M','6M','9M','12M','18M','24M','36M')
and sd.pname = ?
and sd.batch_number = ?
))pivot (max(average_result) for row_no in (1 as a,2 as b,3 as c,4 as d,5 as e,6 as f,7 as g,8 as h))
where ? = case when ? = 'ALL' then all_data else test_parameters end
order by tp_no",
            [$request->pname, $request->batch_number, $request->test_parameters, $request->test_parameters]);
...
where ? = case when ? = 'ALL' then all_data else test_parameters end
order by tp_no",
                [$parameters['pname'], $parameters['batch_number'],$parameters['test_parameters'], $parameters['test_parameters']]);
...
//            dd ($data);
            ...
            \Excel::create('Summary', function ($excel) use ($data) {
                ...
                $excel->sheet('Summary', function ($sheet) use ($data) {
                    $sheet->loadView('ssm_views.reports.result.stabilitymonitoringdata36M2excellayout', $data);
...
order by tp_no",
            ...
            $pdf = \SPDF::loadView('ssm_views.reports.result.stabilitymonitoringdata36M2pdflayout', ['pdata1' => $data_pr,'pname'=>$parameters['pname'],'batch_number'=>$parameters['batch_number']
                ,'KOD'=>$data_pr[0]->kept_on_date,'PS'=>$data_pr[0]->p_size,'MOP'=>$data_pr[0]->mode_of_pack]);
