...
class RequestedApplicationController extends Controller
{
    public function index(){
        ...
        $applicantData = DB::select("select line_id,plant_id,emp_id,emp_name,type_of_leave, purpose_of_leave,status,emp_contact_no,emp_email,leave_from,leave_to,rsp_duties,rsp_accept,rejected_id
           from mis.leave_emp_record
           where rsp_emp_id = ?
           and leave_from between (select trunc(sysdate, 'YEAR') from dual) and (select add_months(trunc(sysdate, 'YEAR'), 12)-1 from dual)
           order by create_date desc", [$uid]);

        return view('elm_portal/requested_application',['applicantData' => $applicantData,'appData' => $appData]);

    }
    public function getLeaveHistoryByYear(Request $request){
        $emp_year = $request->req_year;
        $uid = Auth::user()->user_id;
        if($request->req_year == 'All'){
            $resp_data = DB::select("
            select line_id,plant_id,emp_id,emp_name,purpose_of_leave, status,emp_contact_no,emp_email,leave_from,leave_to, to_char(leave_from,'DD-MM-YYYY') || ' to ' || to_char(leave_to,'DD-MM-YYYY') as days,rsp_duties,rsp_accept,rejected_id
            from mis.leave_emp_record where rsp_emp_id = ? and to_date(to_char(leave_from,'YYYY'),'YYYY') =  decode( ?,'All',to_date(to_char(leave_from,'YYYY'),'YYYY'),?)
            order by line_id desc",[$uid,$emp_year,$emp_year]) ;
            return response()->json($resp_data);
        }else{
            $resp_data = DB::select("
            select line_id,plant_id,emp_id,emp_name, purpose_of_leave,status,emp_contact_no,emp_email,leave_from,leave_to, to_char(leave_from,'DD-MM-YYYY') || ' to ' || to_char(leave_to,'DD-MM-YYYY') as days,rsp_duties,rsp_accept,rejected_id
            from mis.leave_emp_record where rsp_emp_id = ? and to_char(leave_from,'YYYY') =  ?
