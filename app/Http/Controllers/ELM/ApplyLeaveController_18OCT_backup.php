...
class ApplyLeaveController extends Controller
{
    public function index()
    {
        ...
        ");
        if($chkEmp[0]->cnt > 0){
            return view('elm_portal.reports.elm_unauthorimzed');
        }
    ...
    public function checkFacLeaveData(Request $request){
        ...
        if(count($lastLeaveData) > 0){
            ...
            if(count($dates) == $holiday_dates[0]->holiday_cnt){
                ...
                return response()->json(['status'=>1,'holiday_cnt'=>$holiday_dates[0]->holiday_cnt,'frst_holiday'=>$frst_holiday]);
            }else{
...
//     --select distinct m.plant_id, m.weekly_holiday_id,m.effective_date_from,m.effective_date_to, d.day_name, d.holiday
...
//     and m.plant_id = '$request->plant_id' 
//     and d.holiday = 'YES'
//     and to_char (m.effective_date_from, 'RR') =
//                     (select to_char (sysdate, 'RR') from dual)
//     and to_date(m.effective_date_from,'DD-MON-RR') between
//             to_date('$request->st_dt','DD-MON-RR')
//             and to_date('$request->en_dt','DD-MON-RR')                
