...
class Ehf_EntryController extends Controller
{
    //
    // indexEmpWiseRpt
    public function indexEmpWiseRpt()
    {
        return view('emp_history.report.emp_history_report');
    }

    //view form

    public function getEmpWiseRpt()
    {
        $ntf_logs = DB::select("SELECT e.*,f.* ,e.emp_id emp_id,e.sur_name name,nvl(e.emp_type,'no data') as emp_type_d,nvl(f.emp_mail_offi,'no data') as emp_mail_offi_d,
                                       nvl(e.gender,'no data') as empgender,nvl(f.nid,'no nid') as empnid
            FROM mis.emp_his_info e
            LEFT JOIN mis.emp_his_moreinfo f
    ...
    {
        try {
            $password = DB::select('select * from mis.emp_his_credential where user_id = ?', [Auth::user()->user_id]);

            $stat = 'f';
            if ($password) {
                $valid = Hash::check($request->cred, $password[0]->user_pass);
                if ($valid) {
                    $stat = Auth::user()->user_id . 'history';
                }
            }
    ...
    public function user_credential(Request $request)
    {
        ...
        try {
            $check = DB::select('select * from mis.emp_his_credential where user_id = ?', [Auth::user()->user_id]);
            $response = null;

            if ($request->type === 'si') {
                if ($check) {
                    $response = 'Password already set before!';
                } else {
                    $stat = DB::table('mis.emp_his_credential')
                        ->insert([
                            'user_id' => Auth::user()->user_id,
                            'user_pass' => Hash::make($request->cred),
                            'create_date' => Carbon::now()->format('Y-m-d H:m:s')
                        ]);
                    if ($stat) {
            ...
            } else if ($request->type === 'fp') {
                if ($check) {
                    $stat = DB::table('mis.emp_his_credential')
                        ->where('user_id', Auth::user()->user_id)
                        ->update([
                            'user_pass' => Hash::make($request->cred),
                            'update_date' => Carbon::now()->format('Y-m-d H:m:s')
                        ]);
                    ...
                    $response = 'Password Not Set Before.Please signup first!';
                }
            }
    ...
    public function eh_viewForm($valid = 'f')
    {
        if ($valid === Auth::user()->user_id . 'history') {
            $exist = true;
            ...
            $emp_his_moreinfoid = DB::select("select * from mis.EMP_HIS_MOREINFO where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_infoid = DB::select("select * from mis.EMP_HIS_INFO where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_addr = DB::select("select * from mis.EMP_HIS_ADDR where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_hisfamidetail = DB::select("select * from mis.EMP_FAMILY_DETAIL where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_edu_old = DB::select("select * from mis.EMP_HIS_EDUCATION eie,MIS.EDU_DEGREE ed
                                            where emp_id  = ?
                                            and EIE.EDU_DESIG_NAME = ED.DEG_ID
                                            order by ED.PRECEDENCE desc ", [Auth::user()->user_id]);

            $emp_his_emplment_old_ext = DB::select("select * from mis.EMP_HIS_EMPLOYEEMENT where emp_id  = ? and upper(emplo_comp_name) not like 'INCEPTA%' order by emplo_from desc", [Auth::user()->user_id]);
            $emp_his_emplment_old_int = DB::select("select * from mis.EMP_HIS_EMPLOYEEMENT where emp_id  = ? and upper(emplo_comp_name) like 'INCEPTA%' order by emplo_from desc", [Auth::user()->user_id]);
            $emp_his_cbs_old = DB::select("select * from mis.EMP_HIS_CHILDBROSIS where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_pro_quali = DB::select("select * from mis.EMP_HIS_PRO_EQUALI where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_ref_data = DB::select("select * from mis.EMP_HIS_REFERENCE where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_nomi_data = DB::select("select * from mis.EMP_HIS_NOMINEE where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_qurantr_data = DB::select("select * from mis.EMP_HIS_GUARANTOR where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_language = EmpHisLanguage::where('emp_id', '=', Auth::user()->user_id)->orderBy('sl')->get();
            ...
            $emp_his_last_update = DB::select("            
             select MIS.EMP_HISTORY_LAST_UPDATE(?) emp_update from dual "
             , [Auth::user()->user_id]
            );                             


            return view('emp_history.entry_form.emp_his_entry_form',
                compact('login_urright_desig', 'login_moreinfo', 'bd_div', 'bd_dis', 'all_country',
                    ...
                    'edu_all_uni', 'edu_all_sub', 'emp_his_last_update', 'emp_his_language','emp_desig_list','company_name','exist'));
        } else {
            return redirect()->back();
        }
    }

//get present address------districts based on division
    public function getPresentDistrict(Request $request)
    {
        if ($request->ajax()) {

            $pread_div_id = $request->pread_div_id;

            $present_dist = DB::select("select * from mis.bd_districts where division_id=?", [$pread_div_id]);
...
//get permanent address------districts based on division
    public function getPermanentDistrict(Request $request)
    {
        if ($request->ajax()) {

            $pd_div_id = $request->pd_div_id;

            $permanent_dist = DB::select("select * from mis.bd_districts where division_id=?", [$pd_div_id]);
...
//get emergency address------districts based on division
    public function getEmergencyDistrict(Request $request)
    {
        if ($request->ajax()) {

            $emer_div_id = $request->emer_div_id;

            $emer_dist = DB::select("select * from mis.bd_districts where division_id=?", [$emer_div_id]);
    ...
    public function getWholeDistrict(Request $request)
    {
        if ($request->ajax()) {

            $div_id_val = $request->div_id_val;

            $dist_val = DB::select("select * from mis.bd_districts where division_id=?", [$div_id_val]);
...
//getWholeGroup
    public function getWholeGroup(Request $request)
    {
        if ($request->ajax()) {

            $edu_deg_val_id = $request->edu_deg_val_id;

            $degree_val = DB::select("SELECT * FROM MIS.EDU_DEGREE B where deg_id=?", [$edu_deg_val_id]);
            $degree_val_pkid = $degree_val[0]->deg_grp_id;
            $group_val = DB::select("SELECT * FROM MIS.EDU_GROUP B where deg_grp_pkid=?", [$degree_val_pkid]);

            return response()->json(
                [
                    'dis_id' => "9999",
                    'degree_matchid' => $degree_val_pkid,
                    'group_val' => $group_val
                ]);
        }
    }

    public function getWholeDegreeBoard(Request $request)
    {
        if ($request->ajax()) {
            $degree_val_aj = DB::select("SELECT * FROM MIS.EDU_DEGREE B");
            $board_val = DB::select("SELECT E.BOARD_ID, E.BOARD_NAME FROM MIS.EDU_BOARD E");

            return response()->json(
                [
                    'dis_id' => "1_10_2019",
                    'degree_val_aj' => $degree_val_aj,
                    'brd_val_aj' => $board_val
                ]);
        }
    }
...
//            $emp_his_addrid = EmpHisAddress::all();
            $emp_his_addrid = DB::select("select * from mis.EMP_HIS_ADDR where emp_id  = ? ", [Auth::user()->user_id]);
...
//                EmpHistoryMoreInfo::insert($emp_his_moreinfo);
                ...
                if ($emp_his_moreinfoid[0]->emp_final_sub == 'submit_yes') {
                    DB::table('mis.emp_his_addr')->where('emp_id', Auth::user()->user_id)->update([
                        'pre_careof' => $request->fd['careOfPresent'],
                        'pre_country' => $request->fd['countryPresent'],
                        'pre_addr_1st' => $request->fd['villageTownPre1st'],
                        'pre_addr_2nd' => $request->fd['villageTownPre2nd'],
...
//            $emp_his_info = EmpHistoryInfo::all();
            $emp_his_info = DB::select("select * from mis.EMP_HIS_INFO where emp_id  = ? ", [Auth::user()->user_id]);
            if (empty($emp_his_info[0]->emp_id)) {

            } else {
                DB::table('mis.emp_his_info')->where('emp_id', Auth::user()->user_id)->update([
    ...
    public function postPagetwoForm(Request $request)
    {
        if ($request->ajax() && $request) {
            ...
            $emp_his_famideatil = DB::select("select * from mis.EMP_FAMILY_DETAIL where emp_id  = ? ", [Auth::user()->user_id]);
            if (empty($emp_his_famideatil[0]->emp_id)) {
                ...
                DB::table('mis.emp_family_detail')->where('emp_id', Auth::user()->user_id)->update([
...
//            Log::info($request->fdtwo['edu_deg_nam'][0]);
            ...
            $data_edu_avail = DB::select("select * from mis.EMP_HIS_EDUCATION where emp_id  = ? ", [Auth::user()->user_id]);

            $emp_his_moreinfo_t = DB::select("select * from mis.emp_his_moreinfo where emp_id  = ? ", [Auth::user()->user_id]);
            ...
            if ($emp_his_moreinfo_t[0]->emp_final_sub == 'submit_yes') {
                ...
                if (count($data_edu_avail) == 0) {
                    ...
                    DB::table('MIS.EMP_HIS_EDUCATION')->where('emp_id', Auth::user()->user_id)->delete();
                }
            ...
            try {
                $check = EmpHisLanguage::find(Auth::user()->user_id);

                if (count($check) > 0) {
                    EmpHisLanguage::where('emp_id', '=', Auth::user()->user_id)->delete();
                }
...
//            $data_empment_avail = EmpHisEmployeement::all()->where('emp_id', Auth::user()->user_id);
            $data_empment_avail = DB::select("select * from mis.EMP_HIS_EMPLOYEEMENT where emp_id  = ? ", [Auth::user()->user_id]);

            if (count($data_empment_avail) == 0) {

            } else {
                DB::table('MIS.EMP_HIS_EMPLOYEEMENT')->where('emp_id', Auth::user()->user_id)->delete();
            }
...
//            $data_empcbs_avail = EMP_HIS_CHILDBROSIS::all()->where('emp_id', Auth::user()->user_id);
            $data_empcbs_avail = DB::select("select * from mis.EMP_HIS_CHILDBROSIS where emp_id  = ? ", [Auth::user()->user_id]);


            if (count($data_empcbs_avail) == 0) {

            } else {
                DB::table('MIS.EMP_HIS_CHILDBROSIS')->where('emp_id', Auth::user()->user_id)->delete();
...
//            $data_empproquali_avail = EMP_HIS_PRO_QUALI::all()->where('emp_id', Auth::user()->user_id);
            $data_empproquali_avail = DB::select("select * from mis.EMP_HIS_PRO_EQUALI where emp_id  = ? ", [Auth::user()->user_id]);

            if (count($data_empproquali_avail) == 0) {

            } else {
                DB::table('MIS.EMP_HIS_PRO_EQUALI')->where('emp_id', Auth::user()->user_id)->delete();
...
//            $emp_his_ref = EmpHisReference::all()->where('emp_id', Auth::user()->user_id);
            ...
            $emp_his_ref = DB::select("select * from mis.EMP_HIS_REFERENCE where emp_id  = ? ", [Auth::user()->user_id]);
            ...
            if (empty($emp_his_ref[0]->emp_id)) {
                ...
                //update-----------------------------------------
                DB::table('mis.emp_his_reference')->where('emp_id', Auth::user()->user_id)->update([
                    'ref_one_nam' => $request->fdfour['ref_one_name'],
                    'ref_one_desig' => $request->fdfour['ref_one_desig'],
                    'ref_one_mob_no' => $request->fdfour['ref_one_phn_number'],
                    'ref_one_email' => $request->fdfour['ref_one_mail'],
...
//            $data_empnomi_avail = EmpHisNominee::all()->where('emp_id', Auth::user()->user_id);
            $data_empnomi_avail = DB::select("select * from mis.EMP_HIS_NOMINEE where emp_id  = ? ", [Auth::user()->user_id]);
            if (count($data_empnomi_avail) == 0) {

            } else {
                DB::table('MIS.emp_his_nominee')->where('emp_id', Auth::user()->user_id)->delete();
            }
...
//                        'nominee_nam' => $request->fdfour['nominee_nam'][$k_nomi],
            ...
            $login_moreinfo = DB::select("select e.*,f.*,d.*,d.dept_name as emp_dept_name ,initcap(f.desig_name) as design_nam_1st_upper
                           from hrms.emp_information@web_to_hrms e,
                            ...
                            and e.dept_id=d.dept_id", [$login_emp_id]);
...
//            $login_moreinfo[0]->emp_dept_name='DISTRIBUTION';
            if ($login_moreinfo[0]->emp_dept_name == 'DISTRIBUTION' || $login_moreinfo[0]->emp_dept_name == 'CENTRAL WAREHOUSE') {
                ...
                $emp_his_guarantor = DB::select("select * from mis.EMP_HIS_GUARANTOR where emp_id  = ? ", [Auth::user()->user_id]);
...
//                EmpHistoryMoreInfo::insert($emp_his_moreinfo);
                    ...
                    //if exit then do update part--------------------------------------------------
                    DB::table('mis.EMP_HIS_GUARANTOR')->where('emp_id', Auth::user()->user_id)->update([
...
//image upload via ajax request without form or submit button---------------user profile pic-----------------------
...
//        ]);
        ...
        $login_urright_desig = $login_moreinfo[0]->desig_name;
        //get the divisions-------------
        $bd_div = DB::select("SELECT B.DIV_ID, B.DIV_NAME, B.BN_NAME FROM MIS.BD_DIVISONS B ORDER BY DIV_NAME ASC");
        //get the districts-------------
        $bd_dis = DB::select("SELECT B.DIS_ID, B.DIVISION_ID, B.DIS_NAME FROM MIS.BD_DISTRICTS B ORDER BY DIS_NAME ASC");
        //get the all country-------------
        $all_country = DB::select("SELECT A.COUNTRY_ID, A.COUNTRY_CODE, A.COUNTRY_NAME FROM MIS.ALL_COUNTRIES A ORDER BY COUNTRY_NAME ASC");
        $emp_his_moreinfoid = DB::select("select * from mis.EMP_HIS_MOREINFO where emp_id  = ? ", [Auth::user()->user_id]);
        $emp_his_infoid = DB::select("select * from mis.EMP_HIS_INFO where emp_id  = ? ", [Auth::user()->user_id]);
        $emp_his_addr = DB::select("select * from mis.EMP_HIS_ADDR where emp_id  = ? ", [Auth::user()->user_id]);
        $emp_hisfamidetail = DB::select("select * from mis.EMP_FAMILY_DETAIL where emp_id  = ? ", [Auth::user()->user_id]);
        $emp_his_edu_old = DB::select("select * from mis.EMP_HIS_EDUCATION where emp_id  = ? order by edu_passing_yr desc", [Auth::user()->user_id]);
        $emp_his_emplment_old_ext = DB::select("select * from mis.EMP_HIS_EMPLOYEEMENT where emp_id  = ? and upper(emplo_comp_name) not like 'INCEPTA%' order by emplo_from desc", [Auth::user()->user_id]);
        $emp_his_emplment_old_int = DB::select("select * from mis.EMP_HIS_EMPLOYEEMENT where emp_id  = ? and upper(emplo_comp_name) like 'INCEPTA%' order by emplo_from desc", [Auth::user()->user_id]);
        $emp_his_cbs_old = DB::select("select * from mis.EMP_HIS_CHILDBROSIS where emp_id  = ? ", [Auth::user()->user_id]);
        $emp_his_pro_quali = DB::select("select * from mis.EMP_HIS_PRO_EQUALI where emp_id  = ? order by pro_from desc", [Auth::user()->user_id]);
        $emp_his_ref_data = DB::select("select * from mis.EMP_HIS_REFERENCE where emp_id  = ? ", [Auth::user()->user_id]);
        $emp_his_nomi_data = DB::select("select * from mis.EMP_HIS_NOMINEE where emp_id  = ? ", [Auth::user()->user_id]);
        $emp_his_qurantr_data = DB::select("select * from mis.EMP_HIS_GUARANTOR where emp_id  = ? ", [Auth::user()->user_id]);
        ...
        //data----------------------------
        $headerHtml = view()->make('emp_history.entry_form.pdf_header')->render();

        $pdf = \SPDF::setOptions([
            'images' => true,
            'margin-top' => '15mm',
            'header-html' => $headerHtml
        ])->loadView('emp_history/entry_form/emp_his_pdf', compact('login_urright_desig', 'login_moreinfo', 'bd_div', 'bd_dis', 'all_country',
            'emp_his_moreinfoid', 'emp_his_addr', 'emp_his_infoid', 'emp_hisfamidetail',
            'emp_his_edu_old', 'emp_his_cbs_old', 'emp_his_emplment_old_ext','emp_his_emplment_old_int', 'emp_his_pro_quali',
    ...
    public function emphistory_pdf_preview(Request $request)
    {
        ...
        $pdf = \SPDF::setOptions([
            'images' => true
        ])->loadView('emp_history/entry_form/emp_his_pdf_preview', compact('login_urright_desig', 'login_moreinfo', 'bd_div', 'bd_dis', 'all_country',
            'emp_his_moreinfoid', 'emp_his_addr', 'emp_his_infoid', 'emp_hisfamidetail',
            'emp_his_edu_old', 'emp_his_cbs_old', 'emp_his_emplment_old_ext','emp_his_emplment_old_int', 'emp_his_pro_quali',
            'emp_his_ref_data', 'emp_his_nomi_data', 'emp_his_qurantr_data', 'edu_all_board', 'edu_all_grp', 'edu_all_degree','emp_his_language'))->setPaper('a4', 'portrait');
...
//            });
            ...
            $emp_hisfamidetail = DB::select("select * from mis.EMP_FAMILY_DETAIL where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_edu_old = DB::select("select * from mis.EMP_HIS_EDUCATION where emp_id  = ? order by edu_passing_yr desc", [Auth::user()->user_id]);
            $emp_his_emplment_old = DB::select("select * from mis.EMP_HIS_EMPLOYEEMENT where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_cbs_old = DB::select("select * from mis.EMP_HIS_CHILDBROSIS where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_pro_quali = DB::select("select * from mis.EMP_HIS_PRO_EQUALI where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_ref_data = DB::select("select * from mis.EMP_HIS_REFERENCE where emp_id  = ? ", [Auth::user()->user_id]);
            $emp_his_nomi_data = DB::select("select * from mis.EMP_HIS_NOMINEE where emp_id  = ? ", [Auth::user()->user_id]);
            ...
            $pdf = \SPDF::setOptions([
                ...
                'emp_his_moreinfoid', 'emp_his_addr', 'emp_his_infoid', 'emp_hisfamidetail',
                'emp_his_edu_old', 'emp_his_cbs_old', 'emp_his_emplment_old', 'emp_his_pro_quali',
                'emp_his_ref_data', 'emp_his_nomi_data', 'emp_his_qurantr_data', 'edu_all_board', 'edu_all_grp', 'edu_all_degree'))->setPaper('a4', 'portrait');

//            return $pdf->download('Employee_history_Preview.pdf');
...
//                'mime' => 'application/pdf',
            ...
            Mail::send(['html' => 'emp_history.emp_his_mail_pdf'], $data, function ($message) use ($mailaddress, $pdf) {
                $message->to($mailaddress, $mailaddress)
                    ->subject('Employee History Form PDF ');//mail er subject
//                       $message->attachData($pdf, 'filename.pdf');
                $message->attachData($pdf->output(), 'Employee_history_form.pdf', [
                    'mime' => 'application/pdf',
                ]);
                $message->from('Payroll@inceptapharma.com', 'Incepta Employee History Form');///1005975 er name mane supervisor
            });

            return response()->json([
                'select_employeeid' => $mailaddress
//                    $select_emp_id
...
//                'login_employee_mail'=>$login_emp_mail[0]->email
        ...
        if ($request->ajax()) {
            ...
            if ($user) {
                if (strpos($request->email, '@') !== false && strpos($request->email, '.com') !== false) {
                    ...
                    Mail::send(['html' => 'emp_history.emp_his_mail_pdf'], ['user' => $user], function ($message) use ($user, $pdf,$request) {
    ...
    public function nominee_image_upload(Request $request)
    {
        ...
        $extension = $data->getClientOriginalExtension();
        $filename = Auth::user()->user_id . '_nominee' . '.' . $extension;
        ...
        $status = DB::table('mis.emp_his_nominee')->where('emp_id', Auth::user()->user_id)->update([
