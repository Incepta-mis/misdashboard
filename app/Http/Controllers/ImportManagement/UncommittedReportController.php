...
class UncommittedReportController extends Controller
{
    public function index(){
        $poData = DB::select('SELECT distinct ID,PO_NUM from SCM_MATERIAL_PURCHASE_INFO WHERE PO_NUM IS NOT NULL ORDER BY PO_NUM');
        return view('import_portal.uncommittedReport',compact('poData'));
    }
    public function getUncommittedReport(Request $request){
        ...
        $po_num = isset($request->po_num) ? $request->po_num : "";
            ...
            if($due_date_act_from != "" && $due_date_act_to != ""){
                ...
                foreach ($coms as $com){
                    ...
                    foreach ($timeArr as $time){
                        $qry = "SELECT '".$time['month_year']."',COM_OR_PLANT,BDT_VALUE FROM MIS.SCM_MATERIAL_PURCHASE_INFO WHERE TYPE_OF_DOC = 'capital' AND PO_NUM = decode('$po_num','ALL',PO_NUM,'$po_num') AND";
                ...
                foreach ($coms as $com){
                    ...
                    $qry = "SELECT '".$acc_date."' ACCEPTANCE_DATE,COM_OR_PLANT,BDT_VALUE FROM MIS.SCM_MATERIAL_PURCHASE_INFO WHERE TYPE_OF_DOC = 'capital' AND PO_NUM = decode('$po_num','ALL',PO_NUM,'$po_num') AND";
