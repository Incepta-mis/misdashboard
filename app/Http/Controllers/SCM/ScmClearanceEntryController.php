...
class ScmClearanceEntryController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            if(strpos($request->smatname, "'") !== false){
                $resStr = str_replace("'", "''", $request->smatname);
            }else{
                $resStr = $request->smatname;
            }
            $resp_data = DB::Select("select distinct  manufacturer_name
                            from mis.scm_blocklist_material
                            where material_name = '$resStr'
    ...
    public function getAvailQuantity(Request $request){
        if ($request->ajax()) {
            if(strpos($request->_mName, "'") !== false){
                $resStr = str_replace("'", "''", $request->_mName);
            }else{
                $resStr = $request->_mName;
            }
            $_bkl_no = trim($request->_bkl_no);
            $resp_data = DB::Select("select ( sum(qty) - sum(sqty) - SCM_AVL_MATQTY ('$_bkl_no','$resStr') ) qty from
                                        (
                                            select DISTINCT qty ,0 sqty
                                            from mis.scm_blocklist_material
                                            where blocklist_no = '$_bkl_no'
                                            and material_name  = '$resStr'
                                            union all
        ...
        try {
            if ($request->ajax()) {
                ...
                foreach ($request->cr_data as $data) {
                    ...
                    $clr->material_name = urldecode($list->nof_mat);
                    $clr->manufacturer_name = urldecode($list->manufact);
