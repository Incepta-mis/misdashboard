...
class ExpoDatabaseController extends Controller
{
    ...
    public function index()
    {
        ...
        (export_country ||' - '|| product_name  ||' - '|| product_code ||' - '|| plant_id ||' - '|| finish_product_code ||' - '|| product_name ||' - '|| product_generic ||' - '||
    ...
    public function getExpoEntryData(Request $request)
    {
        ...
        $finish_p_code = trim($lovArray[5]);


        //               return response()->json($pack_size);

        // if(!empty($finish_p_code)){
        //     $data = DB::select("
        //     select *
        //     from v_expolov
        //     where plant_id = '$plant_id'
        //     and product_code = '$product_code'
        //     and finish_product_code = '$finish_p_code'
        ...
        if(!empty($finish_p_code)){
            ...
            and product_code = '$product_code'
            and finish_product_code = '$finish_p_code'
            and product_name = q'[$product_name]'
            and export_country = '$export_country'
            and pack_size = (q'[$pack_size]')
            ");
        }else{
...
//        return response()->json($request->all());
        ...
        //     from mis.expo_info
        //     where finish_product_code = '$request->fi_code'
        //     and product_code = '$request->local_pcode'
        //     and export_country = '$request->exp_country'
        //     and pack_size = q'[$pack_size]'
        ...
        if (!empty($request->fi_code)) {
            ...
            and pack_size = q'[$pack_size]'
            and plant_id = '$request->plant_id' 
            and brand_name = q'[$request->brand_name]'
            and expo_product_name = q'[$request->expo_product_name]'            
            order by line_id desc");
...
//            $fileNameToStore = $filename.'-'.time().'.'.$extension;
    ...
    //             $cnt_status = DB::select(" select count(*) cnt from MIS.EXPO_INFO 
    //             where product_code = ? and brand_name = ? 
    //             and export_country = ? and expo_product_name = ?
    //             and pack_size =  q'[$pack_size]' ", [$DataArray[0]->local_pcode, $DataArray[0]->brand_name, $DataArray[0]->exp_country, $DataArray[0]->expo_product_name]);
                
    //             $lines = DB::select(" select max(line_id) line_id 
    //             from mis.expo_info where product_code = ? 
    //             and brand_name = ? and export_country = ? 
    //             and expo_product_name = ?
    //             and pack_size =  q'[$pack_size]' ", 
    //             [$DataArray[0]->local_pcode, $DataArray[0]->brand_name, $DataArray[0]->exp_country, $DataArray[0]->expo_product_name]);
    ...
    public function getIRAStage1Data(Request $request) 
    {
        ...
        if(!empty($DataArray[0]->sub_to_im_date)){
            $im_submitted_date = Carbon::parse($DataArray[0]->sub_to_im_date)->format('Y-m-d');
        }else {
            $im_submitted_date = '';
        }
        ...
        if ($type_of_dossier == 'NON_CTD') {
            ...
            $v_ex_product_name = $DataArray[0]->expo_product_name;
            $v_ex_pack_size = $DataArray[0]->pack_size;
            $v_plant_id = $DataArray[0]->plant_id;
            
            $v_exist = DB::select(" select count(*) cnt 
            from MIS.EXPO_INFO 
            where product_code = ? and brand_name = ? 
            and export_country = ? and pack_size =  q'[$v_ex_pack_size]' and plant_id = ?",
             [$v_local_pcode, $v_ex_product_name, $v_exp_country, $v_plant_id]);
            ...
            if ($v_exist[0]->cnt > 0) {
                ...
                $line_id = $sequence->nextValue('MIS.SEQ_EXPO_INFO');
                if ($DataArray[0]->status == 'Edit') {
                    $MAX_line_id = DB::select("select max(line_id) line_id 
                    from mis.expo_info 
                    where product_code = ? 
                    and brand_name = ? and 
                    export_country = ? and 
                    expo_product_name = ? and pack_size = q'[$v_ex_pack_size]'
                    and plant_id = ? 
                    ", 
                    [$DataArray[0]->local_pcode, $DataArray[0]->brand_name, $DataArray[0]->exp_country, 
                    $DataArray[0]->expo_product_name, $v_plant_id]);
                    ...
                    $insertLineId = DB::select(" select line_id 
                    from ( select line_id,product_code,current_status 
                    from mis.expo_info 
                    where finish_product_code = ? 
                    order by line_id desc ) where rownum <= 1", [$DataArray[0]->finish_product]);

                    $fiCode = DB::select(" select count(*) cnt 
                    from mis.expo_info 
                    where line_id = ? 
                    and current_status = 'REG' ", [$insertLineId[0]->line_id]);
                ...
                $v_ex_product_name = $DataArray[0]->expo_product_name;
                $v_ex_pack_size = $DataArray[0]->pack_size;
                $v_ex_plant_id = $DataArray[0]->plant_id;

                $v_exist = DB::select(" select count(*) cnt 
                from MIS.EXPO_INFO where product_code = ? 
                and brand_name = ? and export_country = ? 
                and pack_size =  q'[$v_ex_pack_size]' and plant_id = ?",
                 [$v_local_pcode, $v_ex_product_name, $v_exp_country, $v_ex_plant_id]);
            ...
            if ($v_exist[0]->cnt > 0) {
                ...
                if ($DataArray[0]->status == 'Edit') {
                    ...
                    $rs = ExpoInfo::where('product_code', $DataArray[0]->local_pcode)
                        ...
                        if ($rs) {
                            $MAX_line_id = DB::select("select max(id) id 
                            from MIS.EXPO_COUNTRY_WISE_PRODUCTS 
                            where product_code = ?
                             and product_name = ? 
                             and pack_size =  q'[$v_ex_pack_size]'
                             and plant_id = ?
                             and export_country = ? ",
                              [$v_local_pcode, $v_ex_product_name,$v_ex_plant_id, $v_exp_country]);
    
    
                            
                              //Log::info($MAX_line_id[0]->id);
    
                              $rss = ExpoCountryWiseProducts::where('product_code', $DataArray[0]->local_pcode)
                              ->where('export_country', $DataArray[0]->exp_country)
                              ->where('id', $MAX_line_id[0]->id)
                              ->update([
                                'finish_product_code' => $DataArray[0]->finish_product,
                              ]);
                    ...
                    $fiCode = DB::select(" select count(*) cnt 
                    from mis.expo_info where line_id = ? 
                    and current_status = 'REG' ", [$insertLineId[0]->line_id]);
        ...
        //RE-Used CTD Working
            ...
            if ($v_exist[0]->cnt > 0) {
                ...
                if ($DataArray[0]->status == 'Edit') {
                    ...
                    if(!empty($DataArray[0]->api_type)){
                        $api_type = $DataArray[0]->api_type;
                    }else{
                        $api_type = '';
                    }

                    if(!empty($filesName[0])){
                        $filesName_0 = $filesName[0];
                    }else{
                        $filesName_0 = '';
                    }

                    if(!empty($filesName[1])){
                        $filesName_1 = $filesName[1];
                    }else{
                        $filesName_1 = '';
                    }

                    if(!empty($filesName[2])){
                        $filesName_2 = $filesName[2];
                    }else{
                        $filesName_2 = '';
                    }

                    if(!empty($filesName[3])){
                        $filesName_3 = $filesName[3];
                    }else{
                        $filesName_3 = '';
                    }
                    if(!empty($DataArray[0]->reuse_pv)){
                        $reuse_pv = $DataArray[0]->reuse_pv;
                    }else{
                        $reuse_pv = '';
                    }
                    if(!empty($DataArray[0]->reuse_impurity_of_fp)){
                        $reuse_impurity_of_fp = $DataArray[0]->reuse_impurity_of_fp;
                    }else{
                        $reuse_impurity_of_fp = '';
                    }
                    if(!empty($DataArray[0]->reuse_ctdp_match)){
                        $reuse_ctdp_match = $DataArray[0]->reuse_ctdp_match;
                    }else{
                        $reuse_ctdp_match = '';
                    }
                    if(!empty($DataArray[0]->reuse_ctd_study)){
                        $reuse_ctd_study = $DataArray[0]->reuse_ctd_study;
                    }else{
                        $reuse_ctd_study = '';
                    }
                    ...
                    if ($fiCode[0]->cnt > 0) {
                        $rs = DB::table('MIS.EXPO_INFO')->insert(
                            [
                                ...
                                'sub_by_ira' => $DataArray[0]->sub_by_ira,
                                'dossier_type' => $DataArray[0]->type_of_dossier,
                                'reuse_api_type' => $api_type,
                                'reuse_ctd_api_source' => $filesName_0,
                                'reuse_ctd_fp_spec' => $filesName_1,
                                'reuse_ctd_manufacturer' => $filesName_2,
                                'reuse_ctd_stability' => $filesName_3,
                                'reuse_pv' => $reuse_pv,
                                'reuse_impurity_of_fp' => $reuse_impurity_of_fp,
                                'reuse_ctdp_match' => $reuse_ctdp_match,
                                'reuse_ctd_study' => $reuse_ctd_study,
                                'create_user' => $uid,
                                'form_status' => 'RE-NEW'
                            ]
                        );
                ...
                $v_local_pcode = $DataArray[0]->local_pcode;
                $v_exp_country = $DataArray[0]->exp_country;
                $v_brand_name = $DataArray[0]->brand_name;
                $v_ex_product_name = $DataArray[0]->expo_product_name;
                $v_ex_pack_size = $DataArray[0]->pack_size;
                $v_ex_plant_id = $DataArray[0]->plant_id;
                $v_exist = DB::select(" select count(*) cnt 
                ...
                and brand_name = ? and export_country = ? 
                and pack_size =  q'[$v_ex_pack_size]' and plant_id = ?",
                [$v_local_pcode, $v_brand_name, $v_exp_country, $v_ex_plant_id]);
                //                $v_exist = DB::select(" select count(*) cnt from MIS.EXPO_INFO where product_code = ? and brand_name = ? and export_country = ? ", [$v_local_pcode, $v_ex_product_name, $v_exp_country]);
        ...
        // without CTD and NON-CTD files
            ...
            from MIS.EXPO_INFO 
            where finish_product_code = ?
            and product_code = ? and brand_name = ?
            and export_country = ? and pack_size =  q'[$pack_size]' and plant_id = ?
            ", [$DataArray[0]->finish_product,$v_local_pcode, $v_ex_product_name,
            $v_exp_country, $v_plant_id]);
            ...
            // Finish Product code Empty
                ...
                where product_code = ? and brand_name = ? 
                and export_country = ? and expo_product_name = ?
                and pack_size =  q'[$pack_size]' and plant_id = ? ",
                [$DataArray[0]->local_pcode, $DataArray[0]->brand_name, 
                $DataArray[0]->exp_country, $DataArray[0]->expo_product_name, $DataArray[0]->plant_id]);
                ...
                and plant_id = ?", 
                [$DataArray[0]->local_pcode, $DataArray[0]->brand_name,
                 $DataArray[0]->exp_country, $DataArray[0]->expo_product_name, 
                 $DataArray[0]->plant_id]);
                ...
                if ($DataArray[0]->status == 'Renew') {
                    ...
                    $fiCode = DB::select("select count(*) cnt from MIS.EXPO_INFO
                                ...
                                and plant_id = ?
                                and current_status = 'Approved'
                                and current_status not in ('Rejected,Pending')", 
                                [$DataArray[0]->finish_product,$DataArray[0]->brand_name,
                                $DataArray[0]->exp_country, $DataArray[0]->expo_product_name,
                                $DataArray[0]->plant_id]);
                    ...
                    // Log::info($DataArray[0]->sub_name);
                    // Log::info($cnt_status[0]->cnt);
    ...
    public function updateStage1IM(Request $request)
    {
        ...
        if (!empty($request->fi_code)) {
            ...
            from MIS.EXPO_INFO where product_code = ? 
            and brand_name = ? and export_country = ?
            and pack_size = q'[$v_pack_size]' ", [$v_local_pcode, $v_ex_product_name, $v_exp_country]);


            if ($v_exist[0]->cnt > 0) {

                $MAX_line_id = DB::select("select max(line_id) line_id from mis.expo_info 
                where product_code = ? and brand_name = ? and 
                export_country = ? and pack_size = q'[$v_pack_size]' ",
                [$v_local_pcode, $v_ex_product_name, $v_exp_country]);
    ...
    public function UPDATE_STAGE2_IM(Request $request)
    {
        ...
        if (!empty($myValue['im_2_remarks'])){
            $im_2_remarks = $myValue['im_2_remarks'];
        }
    ...
    public function UPDATE_STAGE3_IRA(Request $request){
        ...
        //****  Export Update New Interface for IM Team.
        public function getExpoEntryInfoUpdate()
        {
            $expo = DB::select('select distinct export_country
            from mis.expo_info
            order by export_country asc');
            return view('expo_database.expo_update_view', ['expo' => $expo]);
    
    
        }
    
        public function getProductByExpoCountry(Request $request)
        {
            $rs = DB::select("select distinct expo_product_name
            from mis.expo_info
            where export_country = decode(?,'All',export_country,?)
            order by expo_product_name asc", [$request->export_country, $request->export_country]);
    
            return response()->json($rs);
        }
    
        public function getIMExpoData(Request $request)
        {
            $rs = DB::select("select line_id,finish_product_code,product_code,brand_name ,export_country ,
            expo_product_name ,product_generic,pack_size ,company_agent_name ,contact_name,plant_id, renew_status 
            from mis.expo_info
            where export_country = decode(?,'All',export_country,?) 
            and expo_product_name = decode(?,'All',expo_product_name,?)",
                [$request->export_country, $request->export_country, $request->expo_product_name, $request->expo_product_name]);
            return response()->json($rs);
        }
    
        public function getLineWiseIMData($line_id)
        {
            $rs = DB::select('select *        
            from mis.expo_info
            where line_id = ?', [$line_id]);
    
            return view('expo_database.expo_update_details', compact('rs'));
        }
    
        public function saveImStage1(Request $request)
        {
            ...
            $rs = ExpoInfo::where('line_id', $request->line_id)
                ->update([
                    'company_agent_name' => $request->name_of_comp_agent,
                    'name_of_the_company_agent' => $request->name_of_comp_agent,
                    'contact_name' => $request->contact_name,
                    'submitted_to_agent' => $sub_to_agent_date,
                    'submitted_to_regularity' => $sub_to_regulatory,
                    'update_user_im_stage1' => $request->auth_id,
                    'update_date_im_stage1' => $sys_time
                ]);
        ...
        public function saveIRAStage2(Request $request)
        {
            $sys_time = Carbon::now('Asia/Dhaka')->format('Y-m-d H:i:s');
            if (!empty($request->in_process_dg_date)) {
                $in_process_dg_date = Carbon::parse($request->in_process_dg_date)->format('Y-m-d');
            } else {
                $in_process_dg_date = '';
            }
    
            if (!empty($request->dg_date_close)) {
                $dg_date_close = Carbon::parse($request->dg_date_close)->format('Y-m-d');
            } else {
                $dg_date_close = '';
            }
            ...
            $rs = ExpoInfo::where('line_id', $request->line_id)
                ->update([
                    'in_process_def_gen' => $request->in_process_def_gen,
                    'in_process_dg_date' => $in_process_dg_date,
                    'dg_comments' => $request->dg_comments,
                    'def_gen_close' => $request->def_gen_close,
                    'dg_date_close' => $dg_date_close,
                    'inprocess' => $request->inprocess,
                    'update_user_ira_stage2' => $request->auth_id,
                    'update_date_ira_stage2' => $sys_time
                ]);
        ...
        // public function saveIMStage2(Request $request)
        // {
        ...
        //         if(file_exists( public_path() . '/IRA_PDF/RegistrationCertificate/'. $filenameWithExt)){
        //             unlink( public_path() . '/IRA_PDF/RegistrationCertificate/'. $filenameWithExt);
        //         }
        //         $files->move($destinationPath, $filenameWithExt); // move files to destination folder
        //         $line_path = '/IRA_PDF/RegistrationCertificate/' . $filenameWithExt;

        //         /*foreach ($files as $file) {//this statement will loop through all files.
        //             $file_name = $file->getClientOriginalName(); //Get file original name//Log::info($file);

        //             if(file_exists( public_path() . $myValue['reg_cert_path'])){
        //                 unlink( public_path() . $myValue['reg_cert_path'] );
        //             }
        //             $file->move($destinationPath, $file_name); // move files to destination folder
        //         }*/
        ...
        //                    $im_2_remarks = $myValue['im_2_remarks'];
        //                }
        //                if (!empty($myValue['reg_shelf_life_product'])) {
        //                    $regSelfLifeProd = $myValue['reg_shelf_life_product'];
        //                }

        //                if(!empty($line_path)){
        //                    $rs = ExpoInfo::where('line_id', $line_id)
        //                        ->update([
        //                            'dropped_by_agent_mah' => $mah_date,
        //                            'withdrawal_form_ra_date' => $wfr_date,
        //                            'approval_date' => $approval_date,
        ...
        public function saveIMStage2(Request $request)
        {
            ...
            if (!empty($myValue['permitted_date'])) {
                $permitted_date = Carbon::parse($myValue['permitted_date'])->format('Y-m-d');
            }
            if (!empty($myValue['expiry_date'])) {
                $expiry_date = Carbon::parse($myValue['expiry_date'])->format('Y-m-d');
            }
        ...
        public function saveIRAStage3(Request $request){
            $sys_time = Carbon::now('Asia/Dhaka')->format('Y-m-d H:i:s');
    
            $rs = ExpoInfo::where('line_id', $request->line_id)
                ->update([
                    'spl_instruction' => $request->spl_instruction,
                    'post_marketing_commitments' => $request->pmc,
                    'time_req_for_reg' => $request->time_req_for_reg,
                    'remarks_by_ira' => $request->remarks_by_ira,
                    'update_user_ira_stage2' => $request->auth_id,
                    'update_date_ira_stage2' => $sys_time
                ]);
