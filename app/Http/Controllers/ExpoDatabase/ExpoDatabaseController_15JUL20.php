...
class ExpoDatabaseController extends Controller
{
    ...
    {
//        $data = DB::select("
//            select ep.plant_id,ep.product_code,ep.product_name, ep.export_country,ep.finish_product_code,
//            (ep.product_code ||' - '||dom.product_name||' - '|| ep.finish_product_code ||' - '|| ep.product_name ||' - '|| ep.product_generic ||' - '||
//            ep.export_country ||' - '|| ep.pack_size ||' - '|| ep.company_agent_name ||' - '|| ep.contact_name  ||' - '|| ep.address ) details
//            from MIS.EXPO_COUNTRY_WISE_PRODUCTS ep, MIS.EXPO_PLANT_WISE_PRODUCTS dom, MIS.EXPO_PLANT pl
//            where ep.plant_id = dom.plant_id
//            and ep.product_code = dom.product_code
//            and ep.plant_id = pl.plant_id
//        ");


        $data = DB::select("
        select distinct  export_country,plant_id plant_id,product_code,product_name, finish_product_code,
...
//            $fileNameToStore = $filename.'-'.time().'.'.$extension;
    ...
    public function getIRAStage1Data(Request $request)
    {
        ...
        if ($type_of_dossier == 'NON_CTD') {
            ...
            from MIS.EXPO_INFO where product_code = ? and brand_name = ? and export_country = ? ", [$v_local_pcode, $v_ex_product_name, $v_exp_country]);
