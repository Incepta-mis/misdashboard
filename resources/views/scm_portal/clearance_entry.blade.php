            ...
            $('#itemName').on('change', function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        for (var l = 0; l< response.length; l++) {
                            var id = encodeURI(response[l]['material_name']);
            ...
            $('.matName').on('change', function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        for (var l = 0; l< response.length; l++) {
                            var id = encodeURI(response[l]['manufacturer_name']);
            ...
            $('#sv').on('click', function (e) {
                ...
                if (_qty === '' || _uom === '' || _itemName === null || _matName === null || _mnf == null) {
                    ...
                    if(duplicateFound){
                        ...
                        // var matNameDec = decodeURI($('.matName').val());
                        //     matNameDec = matNameDec.replace(/(['"])/g, "\\$1");
                        //
                        // var manufactNameDec = decodeURI($('.manufactName').val());
                        //     manufactNameDec = manufactNameDec.replace(/(['"])/g, "\\$1");

                        var matNameDec = $('.matName').val();
                        var manufactNameDec = $('.manufactName').val();

                        $("#cl_fn_list").append('<tr valign="top">' +
                            '<td><input type="text"  class="b_name" name="bkln_list" value="' + $('.itemName').val() + '" readonly/> </td>' +
                            '<td><input type="hidden"  class="m_name" name="nof_mat" value="' + matNameDec + '" ' +
                            'readonly/> <span style="font-size: 11px; display: block; width: 114px;' +
                            'word-wrap:break-word; border: 1px solid black; background: white; margin: 5px; ' +
                            'white-space: break-spaces;">'+decodeURI(matNameDec)+'</span></td>' +
                            '<td><input type="hidden"  name="manufact" value="' + manufactNameDec + '" readonly/> ' +
                            '<span style="font-size: 11px; display: block; width: 114px;' +
                            'word-wrap:break-word; border: 1px solid black; background: white; margin: 5px; ' +
                            'white-space: break-spaces;">'+decodeURI(manufactNameDec)+'</span></td>' +
