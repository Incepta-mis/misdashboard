...
@section('title','Summary of Stability Monitoring Data Export Country_03')
@section('styles')
        ...
        $(document).ready(function () {
            ...
            function preparereport1(values) {
                ...
                for (var i = 0; i < values.length; i++) {
                    ...
                    table += '<tr><td>' + values[i].test_parameters + '</td><td>' + values[i].accept_criteria + '</td><td>' + ini1 + '</td><td>' + ini2 + '</td><td>' + ini3 + '</td><td>' + ini4 + '</td><td>' + ini5 + '</td><td>' + ini6 + '</td><td>' + ini7 + '</td></tr>'
                }
                table += '</tbody></table>';
                return table;
            ...
            $('#pname').on('change', function () {
                if ($(this).val() !== 'ALL') {
                    $.ajax({
                        type: 'get',
                        url: '{{url('ssm/report/result/batchget25')}}',
                        data: {
            ...
            $('#btn_display').on('click', function () {
                ...
                $.ajax({
                    ...
                    type: 'post',
                    url: '{{url('ssm/report/result/displayresult25')}}',
                    data: {
                            ...
                            buttons: [
                                {
                                    ...
                                    action: function () {
                                        ...
                                        window.location = '{{URL::to('ssm/report/result/exportexcel26/')}}' + '/' + $('#stabilitymonitoringdataecountry04_info').serialize();
                                    }
                                },
                                {
                                    text: 'PDF',
                                    ...
                                    action: function () {
                                        // alert($('#sample_info_data').serialize());
                                        window.location = '{{URL::to('ssm/report/result/exportpdf26/')}}' + '/' + $('#stabilitymonitoringdataecountry04_info').serialize();
                                    }
                                }
