...
@section('title','Summary of Stability Monitoring Data for PD Batch_03')
@section('styles')
    ...
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <section class="panel" id="data_table">
                <form action="" id="stabilitymonitoringdatapdbatch03_info_data">
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12">
                                <fieldset class="scheduler-border">
                                    <legend class="scheduler-border">Summary of Stability Monitoring Data for PD Batch_03(Selection Option)</legend>
                                    <form id="selection">
                                        <div class="col-md-12">
                                            <div class="col-md-3">
        ...
        $(document).ready(function () {
            ...
            function preparereport1(values)
            {
                ...
                table +=' <thead>\n' +
                    ...
                    '            <th colspan="3" style="text-align: center;vertical-align: middle">Stability Time Points (months)</th>\n' +
                    '        </tr>\n' +
                    '        <tr>\n' +
                    '            <th colspan="3" class="text-center">Long Term Stability Data  <br> (30 °C ± 2 °C/65% RH ± 5% RH)</th>\n' +
                ...
                for (var i=0;i<values.length;i++){
                    ...
                    var ini4 = values[i]['4'] ? values[i]['4'] : '';
                    var ini5 = values[i]['5'] ? values[i]['5'] : '';
                    table += '<tr><td>'+values[i].test_parameters+'</td><td>'+values[i].accept_criteria+'</td><td>'+ini1+'</td><td>'+ini2+'</td><td>'+ini3+'</td><td>'+ini4+'</td><td>'+ini5+'</td></tr>'
                }
                table +='</tbody></table>';
                return table;
            ...
            $('#pname').on('change', function () {
                if ($(this).val() !== 'ALL') {
                    $.ajax({
                        type: 'get',
                        url: '{{url('ssm/report/result/batchget19')}}',
                        data: {
            ...
            $('#btn_display').on('click', function () {
                ...
                $.ajax({
                    ...
                    type: 'post',
                    url: '{{url('ssm/report/result/displayresult19')}}',
                    data: {
                            ...
                            buttons: [
                                {
                                    ...
                                    action: function () {
                                        ...
                                        window.location = '{{URL::to('ssm/report/result/exportexcel20/')}}' + '/' + $('#stabilitymonitoringdatapdbatch03_info_data').serialize();
                                    }
                                },
                                {
                                    text: 'PDF',
                                    ...
                                    action: function () {
                                        // alert($('#sample_info_data').serialize());
                                        window.location = '{{URL::to('ssm/report/result/exportpdf20/')}}' + '/' + $('#stabilitymonitoringdatapdbatch03_info_data').serialize();
                                    }
                                }
