...
@section('title','Analyst Wise Product List')
@section('styles')
    ...
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <section class="panel" id="data_table">
                <form action="" id="analystwiseprdctlist_info_data">
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12">
                                <fieldset class="scheduler-border">
                                    <legend class="scheduler-border">Analyst Wise Product List(Selection Option)
                                    </legend>
                                    <form id="selection">
                                        <div class="col-md-12">
                                            <div class="col-md-2">
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    ...
                                                    <select name="m_a_to" id="m_a_to"
                                                            class="form-control input-xs">
                                                        <option value="ALL">All</option>
                                                        @foreach($m_a_to as $mat)
                                                            <option value="{{$mat->m_a_to}}">{{$mat->m_a_to}}</option>
                                                        @endforeach
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    ...
                                                    <select name="chamber_stor_cond" id="chamber_stor_cond"
                                                            class="form-control input-xs">
                                                        <option value="ALL">All</option>
                                                        @foreach($chamber_stor_cond as $csl)
                                                            <option value="{{$csl->chamber_stor_cond}}">{{$csl->chamber_stor_cond}}</option>
                                                        @endforeach
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    <label for="email"><b>Time Point:</b></label>

                                                    <select name="time_point" id="time_point"
                                                            class="form-control input-xs">
                                                        <option value="ALL">All</option>
                                                        @foreach($time_point as $tp)
                                                            <option value="{{$tp->time_point}}">{{$tp->time_point}}</option>
                                                        @endforeach
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function preparereport1(values) {
                ...
                table += '<thead><tr><th>SL No.</th><th>Product Name</th><th>Batch</th><th>Analyst Name</th><th>Kept On Date</th><th>Time Points</th><th>Chamber Storage Condition</th><th>Stab Types</th></tr></thead>';
                table += '<tbody> ';
                var c = 0;
                for (var i = 0; i < values.length; i++) {
                    c++;
                    var remarks = values[i].remarks ? values[i].remarks : '';
                    table += '<tr><td>' + c + '</td><td>' + values[i].pname + '</td><td>' + values[i].batch_number + '</td><td>' + (values[i].analyst_name ? values[i].analyst_name: '') + '</td><td>' + values[i].kept_on_date + '</td><td>' + values[i].time_point + '</td><td>' + values[i].chamber_stor_cond + '</td><td>' + values[i].stab_type + '</td></tr>'
                }
                table += '</tbody></table>';
                return table;
            ...
            $('#pname').on('change', function () {
                if ($(this).val() !== 'ALL') {
                    $.ajax({
                        type: 'get',
                        url: '{{url('ssm/report/sample/batchget2')}}',
                        data: {
                        ...
                        success: function (response) {
                            ...
                            $('#m_a_to').empty().append(getoptionlist(response.m_a_to));
                            $('#time_point').empty().append(getoptionlist(response.time_point));
                            // $('#test_parameters').empty().append(getoptionlist(response.test_parameters));
                            ...
                            buttons: [
                                {
                                    ...
                                    action: function () {
                                        ...
                                        window.location = '{{URL::to('ssm/report/sample/exportexcel3/')}}' + '/' + $('#analystwiseprdctlist_info_data').serialize();
                                    }
                                },
                                {
                                    text: 'PDF',
                                    ...
                                    action: function () {
                                        // alert($('#sample_info_data').serialize());
                                        window.location = '{{URL::to('ssm/report/sample/exportpdf3/')}}' + '/' + $('#analystwiseprdctlist_info_data').serialize();
                                    }
                                }
