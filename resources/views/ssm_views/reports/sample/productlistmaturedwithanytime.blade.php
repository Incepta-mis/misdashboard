...
@section('title','Product List Matured Within Anytime')
@section('styles')
    ...
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <section class="panel" id="data_table">
                <form action="" id="productlistmaturewithinanytime_info_data">
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12">
                                <fieldset class="scheduler-border">
                                    <legend class="scheduler-border">Product List Matured Within Anytime(Selection
                                        Option)
                                    </legend>
                                    <form id="selection">
                                        <div class="col-md-12">
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    <label for="email"><b>Kept On Date+Days</b></label>
                                                    <input type="text" onkeyup="this.value = this.value.toUpperCase();"
                                                           class="form-control input-xs"
                                                           id="days" placeholder="" name="days">
                                                </div>
                                            </div>
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    <label for="email"><b>Kept On Date+Months</b></label>
                                                    <input type="text" onkeyup="this.value = this.value.toUpperCase();"
                                                           class="form-control input-xs"
                                                           id="months" placeholder="" name="months">
                                                </div>
                                            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function preparereport1(values) {
                ...
                table += '<thead><tr><th>SL No.</th><th>Product Name</th><th>Batch</th><th>Kept On Date</th><th>Chamber ID</th><th>pulling Time Point</th><th>Chamber Storage Condition</th><th>Chamber Storage  Location</th><th>Initial sample Qty</th><th>Remaining sample Qty</th><th>Remarks</th></tr></thead>';
                table += '<tbody> ';
                var c = 0;
                for (var i = 0; i < values.length; i++) {
                    c++;
                    var remarks = values[i].remarks ? values[i].remarks : '';
                    table += '<tr><td>' + c + '</td><td>' + values[i].pname + '</td><td>' + values[i].batch_number + '</td><td>' + values[i].kept_on_date + '</td><td>' + values[i].chamber_id + '</td><td>' + values[i].chamber_stor_cond + '</td><td></td><td>' + values[i].chamber_stor_loc + '</td><td>' + values[i].initial_sample_qty + '</td><td></td><td>' + values[i].rem_sample_qty + '</td><td>' + remarks + '</td></tr>'
                }
                table += '</tbody></table>';
                return table;
            ...
            $('#pname').on('change', function () {
                if ($(this).val() !== 'ALL') {
                    $.ajax({
                        type: 'get',
                        url: '{{url('ssm/report/sample/batchget5')}}',
                        data: {
            ...
            $('#btn_display').on('click', function () {
                ...
                $.ajax({
                    ...
                    type: 'post',
                    url: '{{url('ssm/report/sample/displayresult5')}}',
                    data: {
                        pname: $('#pname').val(),
                            ...
                            buttons: [
                                {
                                    ...
                                    action: function () {
                                        ...
                                        window.location = '{{URL::to('ssm/report/sample/exportexcel6/')}}' + '/' + $('#productlistmaturewithinanytime_info_data').serialize();
                                    }
                                },
                                {
                                    text: 'PDF',
                                    ...
                                    action: function () {
                                        // alert($('#sample_info_data').serialize());
                                        window.location = '{{URL::to('ssm/report/sample/exportpdf6/')}}' + '/' + $('#productlistmaturewithinanytime_info_data').serialize();
                                    }
                                }
