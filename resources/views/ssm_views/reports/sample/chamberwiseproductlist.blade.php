...
@section('title','Chamber Wise Product List')
@section('styles')
    ...
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <section class="panel" id="data_table">
                <form action="" id="timewiseprdctlist_info_data">
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12">
                                <fieldset class="scheduler-border">
                                    <legend class="scheduler-border">Chamber Wise Product List(Selection Option)
                                    </legend>
                                    <form id="selection">
                                        <div class="col-md-12">
                                            <div class="col-md-12" style="margin-bottom: 4px;">
                                                <div class="form-group">
                                                    ...
                                                    <select name="spname" multiple="multiple" id="spname" class="form-control input-xs">
...
{{--                                                        @endforeach--}}
                                                    ...
                                                    <input type="hidden" name="pname" id="pname" value="ALL">
                                                </div>
                                            </div>
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    ...
                                                    <select name="chamber_id" id="chamber_id"
                                                        ...
                                                        @foreach($chamber_id as $cid)
                                                            <option value="{{$cid->chamber_id}}">{{$cid->chamber_id}}</option>
                                                        @endforeach
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    ...
                                                    <select name="chamber_stor_loc" id="chamber_stor_loc"
                                                            class="form-control input-xs">
                                                        <option value="ALL">All</option>
                                                        @foreach($chamber_stor_loc as $csl)
                                                            <option value="{{$csl->chamber_stor_loc}}">{{$csl->chamber_stor_loc}}</option>
                                                        @endforeach
        ...
        <script>
            $(document).ready(function () {
                ...
                function preparereport1(values) {
                    ...
                    table += '<thead><tr><th>SL No.</th><th>Product Name</th><th>Batch</th><th>Kept On Date</th><th>Chamber ID</th><th>Chamber Storage Condition</th><th>Chamber Storage  Location</th><th>Initial sample Qty</th><th>Remaining sample Qty</th><th>Remarks</th></tr></thead>';
                    table += '<tbody> ';
                    var c = 0;
                    for (var i = 0; i < values.length; i++) {
                        c++;
                        var remarks = values[i].remarks ? values[i].remarks : '';
                        var cond = values[i].chamber_stor_cond ? values[i].chamber_stor_cond : '';
                        var loc = values[i].chamber_stor_loc ? values[i].chamber_stor_loc : '';
                        var isq = values[i].initial_sample_qty ? values[i].initial_sample_qty : '';
                        var rsq = values[i].rem_sample_qty ? values[i].rem_sample_qty : '';
                        table += '<tr><td>' + c + '</td><td>' + values[i].pname + '</td><td>' + values[i].batch_number + '</td><td>' + values[i].kept_on_date + '</td><td>' + values[i].chamber_id + '</td><td>' + cond + '</td><td>' + loc + '</td><td>' + isq + '</td><td>' + rsq + '</td><td>'+remarks+'</td></tr>'
                    }
                    table += '</tbody></table>';
                    return table;
                ...
                function getSelectedData(){
                    var selectedProducts = $("#spname").select2("data");
                    // console.log(category)
                    let products = [];
                    selectedProducts.forEach(function (row){
                        console.log(row.text);
                        products.push(row.text);
                    });
                ...
                $('#spname').select2({
                    ...
                    ajax: {
                        url: '{{url('ssm/report/sample/searchProduct')}}',
                    ...
                    var products = getSelectedData();

                    //$('#pname').val(products.join(`','`));

                    const containsAll = products.find((product)=> product === 'ALL');

                    if(containsAll){
                        console.log('selected all')
                        $('#spname').val(['ALL']).trigger('change');
                        $('#pname').val('ALL');
                    }else{
                        $('#pname').val(products.join(`','`));
                    }
                    ...
                    if(products.length === 1){
                        //for single product fetch other select values
                        $('#pname').val(products[0]);
                        if(products[0] !== 'ALL'){
                            $.ajax({
                                type: 'get',
                                url: '{{url('ssm/report/sample/batchget4')}}',
                                data: {
                                ...
                                success: function (response) {
                                    ...
                                    $('#chamber_id').empty().append(getoptionlist(response.chamber_id));
                                    $('#chamber_stor_cond').empty().append(getoptionlist(response.cond));
                                    $('#chamber_stor_loc').empty().append(getoptionlist(response.loc));
                ...
                $('#btn_display').on('click', function () {
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        url: '{{url('ssm/report/sample/displayresult4')}}',
                        data: {
                            pname: $('#pname').val(),
                            chamber_id: $('#chamber_id').val(),
                            batch_number: $('#batch_number').val(),
                            chamber_stor_cond: $('#chamber_stor_cond').val(),
                            chamber_stor_loc: $('#chamber_stor_loc').val(),
                                ...
                                buttons: [
                                    {
                                        ...
                                        action: function () {
                                            ...
                                            window.location = '{{URL::to('ssm/report/sample/exportexcel5/')}}' + '/' + encodeURIComponent($('#timewiseprdctlist_info_data').serialize());
                                        }
                                    },
                                    {
                                        text: 'PDF',
                                        ...
                                        action: function () {
                                            // alert($('#sample_info_data').serialize());
                                            window.location = '{{URL::to('ssm/report/sample/exportpdf5/')}}' + '/' +  encodeURIComponent($('#timewiseprdctlist_info_data').serialize());
                                        }
                                    }
