...
@section('title','Machine Utilization Calculation')
@section('styles')
    ...
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <section class="panel" id="data_table">
                <form action="" id="machineutical_info_data">
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12">
                                <fieldset class="scheduler-border">
                                    <legend class="scheduler-border">Machine Utilization Calculation(Selection Option)</legend>
                                    <form id="selection">
                                        <div class="col-md-12">
                                            <div class="col-md-3">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function preparereport1(values)
            {
                ...
                table +='<thead><tr><th>SL No.</th><th>Machine ID</th><th>Product Name</th><th>Batch</th><th>Machine Start  Date Time</th><th>Machine Stop Date Time</th><th>Machine Run Time</th><th>Remarks</th></tr></thead>';
                table += '<tbody> ';
                var c = 0;
                for (var i=0;i<values.length;i++){
                    c++;
                    var remarks = values[i].remarks ? values[i].remarks : '';
                    var masdt = values[i].msdt ? values[i].msdt : '';
                    table += '<tr><td>'+c+'</td><td>'+values[i].m_id+'</td><td>'+values[i].pname+'</td><td>'+values[i].batch_number+'</td><td>'+values[i].m_start_date_time+'</td><td>'+values[i].m_stop_date_time+'</td><td>'+values[i].tot_mh+'</td><td>'+remarks+'</td></tr>'
                }
                table +='</tbody></table>';
                return table;
            ...
            $('#m_id').on('change', function () {
                if ($(this).val() !== 'ALL') {
                    $.ajax({
                        type: 'get',
                        url: '{{url('ssm/report/sample/batchget1')}}',
                        data: {
            ...
            $('#btn_display').on('click', function () {
                ...
                $.ajax({
                    ...
                    type: 'post',
                    url: '{{url('ssm/report/sample/displayresult1')}}',
                    data: {
                        m_id: $('#m_id').val(),
                        pname: $('#pname').val(),
                            ...
                            buttons: [
                                {
                                    ...
                                    action: function () {
                                        ...
                                        window.location = '{{URL::to('ssm/report/sample/exportexcel2/')}}' + '/' + $('#machineutical_info_data').serialize();
                                    }
                                },
                                {
                                    text: 'PDF',
                                    ...
                                    action: function () {
                                        // alert($('#sample_info_data').serialize());
                                        window.location = '{{URL::to('ssm/report/sample/exportpdf2/')}}' + '/' + $('#machineutical_info_data').serialize();
                                    }
                                }
