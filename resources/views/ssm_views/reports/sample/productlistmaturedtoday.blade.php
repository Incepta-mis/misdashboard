...
@section('title','Product List Matured Today')
@section('styles')
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <fieldset class="scheduler-border">
                                    <legend class="scheduler-border">Product List Matured Today(Selection Option)
                                    </legend>
                                    <form id="selection">
                                        <div class="col-md-12">
                                            <div class="col-md-2">
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                function preparereport1(values) {
                    ...
                    table += '<thead><tr><th>SL No.</th><th>Product Name</th><th>Batch</th><th>Kept On Date</th><th>Pulling Date</th><th>pulling Time Point</th><th>Chamber ID</th><th>Chamber Storage Condition</th><th>Chamber Storage  Location</th><th>Initial sample Qty</th><th>Last sample Qty</th><th>Remarks</th></tr></thead>';
                    table += '<tbody> ';
                    var c = 0;
                    for (var i = 0; i < values.length; i++) {
                        c++;
                        var remarks = values[i].remarks ? values[i].remarks : '';
                        var pullingdate = values[i].pulling_date ? values[i].pulling_date : '';
                        var loc = values[i].chamber_stor_loc ? values[i].chamber_stor_loc : '';
                        var iniqty = values[i].initial_sample_qty ? values[i].initial_sample_qty : '';
                        var lastqty = values[i].rem_sample_qty ? values[i].rem_sample_qty : '';
                        table += '<tr><td>' + c + '</td><td>' + values[i].pname + '</td><td>' + values[i].batch_number + '</td><td>' + values[i].kept_on_date + '</td><td>' + pullingdate + '</td><td>' + values[i].pulling_time_point + '</td><td>' + values[i].chamber_id + '</td><td>' + values[i].chamber_stor_cond + '</td><td>' + loc+ '</td><td>' + iniqty + '</td><td>' + lastqty + '</td><td>'+remarks+'</td></tr>'
                    }
                    table += '</tbody></table>';
                    return table;
                ...
                $('#pname').on('change', function () {
                    if ($(this).val() !== 'ALL') {
                        $.ajax({
                            type: 'get',
                            url: '{{url('ssm/report/sample/batchget6')}}',
                            data: {
                ...
                $('#btn_display').on('click', function () {
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        url: '{{url('ssm/report/sample/displayresult6')}}',
                        data: {
                            pname: $('#pname').val(),
                                ...
                                buttons: [
                                    {
                                        ...
                                        action: function () {
                                            ...
                                            window.location = '{{URL::to('ssm/report/sample/exportexcel7/')}}' + '/' + $('#timewiseprdctlist_info_data').serialize();
                                        }
                                    },
                                    {
                                        text: 'PDF',
                                        ...
                                        action: function () {
                                            // alert($('#sample_info_data').serialize());
                                            window.location = '{{URL::to('ssm/report/sample/exportpdf7/')}}' + '/' + $('#timewiseprdctlist_info_data').serialize();
                                        }
                                    }
