...
@section('title','Time Point Wise Product List')
@section('styles')
    ...
    <div class="row">
        <div class="col-sm-12 col-md-12">
            <section class="panel" id="data_table">
                <form action="" id="timepointwiseprdctlist_info_data">
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-12">
                                <fieldset class="scheduler-border">
                                    <legend class="scheduler-border">Time Point Wise Product List(Selection Option)
                                    </legend>
                                    <form id="selection">
                                        <div class="col-md-12">
                                            <div class="col-md-2">
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    <label for="email"><b>Pulling Month:</b></label>

                                                    <select name="pulling_month" id="pulling_month"
                                                            class="form-control input-xs">
{{--                                                        <option value="ALL">All</option>--}}
                                                        @foreach($month_name as $ma)
                                                            <option value="{{$ma->monthname}}">{{$ma->monthname}}</option>
                                                        @endforeach
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                function preparereport1(values) {
                    ...
                    table += '<thead><tr><th>SL No.</th><th>Product Name</th><th>Batch</th><th>Kept On Date</th><th>Pulling Date</th><th>pulling Time Point</th><th>Chamber ID</th><th>Chamber Storage Location</th><th>Chamber Storage Condition</th><th>Initial sample Qty</th><th>Remaining sample Qty</th><th>Remarks</th></tr></thead>';
                    table += '<tbody> ';
                    var c = 0;
                    for (var i = 0; i < values.length; i++) {
                        c++;
                        ...
                        table += '<tr><td>' + c + '</td><td>' + values[i].pname + '</td><td>' + values[i].batch_number + '</td><td>' + values[i].kept_on_date + '</td><td>' + values[i].pulling_date + '</td><td>' + values[i].pulling_time_point + '</td><td>' + values[i].chamber_id + '</td><td>' + values[i].chamber_stor_loc + '</td><td>' + values[i].chamber_stor_cond + '</td><td>' + values[i].initial_sample_qty + '</td><td>' + rsq + '</td><td>'+remarks+'</td></tr>'
                    }
                    table += '</tbody></table>';
                    return table;
                ...
                $('#pname').on('change', function () {
                    if ($(this).val() !== 'ALL') {
                        $.ajax({
                            type: 'get',
                            url: '{{url('ssm/report/sample/batchget3')}}',
                            data: {
                ...
                $('#btn_display').on('click', function () {
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        url: '{{url('ssm/report/sample/displayresult3')}}',
                        data: {
                            pname: $('#pname').val(),
                            ...
                            pulling_month: $('#pulling_month').val(),
                            _token: '{{csrf_token()}}'
                        },
                        success: function (response) {
                            console.log(response);
                                ...
                                buttons: [
                                    {
                                        ...
                                        action: function () {
                                            ...
                                            window.location = '{{URL::to('ssm/report/sample/exportexcel4/')}}' + '/' + $('#timepointwiseprdctlist_info_data').serialize();
                                        }
                                    },
                                    {
                                        text: 'PDF',
                                        ...
                                        action: function () {
                                            // alert($('#sample_info_data').serialize());
                                            window.location = '{{URL::to('ssm/report/sample/exportpdf4/')}}' + '/' + $('#timepointwiseprdctlist_info_data').serialize();
                                        }
                                    }
