        ...
        $(document).ready(function () {
                            ...
                            buttons: [
                                {
                                    ...
                                    action: function (e, dt, node, config) {
                                        ...
                                        if (deldata.length !== 0) {
                                            ...
                                            var datatt = xxtable.rows().data();
                                            var dataAsArray = [];
                                            // console.log(data.length);

                                            for (var i = 0; i < datatt.length; i++) {
                                                dataAsArray.push(datatt[i]);
                                            }
                                            $(deldata).each(function(key,val){
                                                // console.log(val.emp_id);
                                                $(dataAsArray).each(function(key2,val2){
                                                    if(val2.emp_id==val.emp_id){
                                                       flag=1;
                                                       alert(val2.emp_id+" employee already exist.please do not select this employee");
                                                    }
                                                })
                                            })
                                            // console.log(deldata);
                                            if(flag==0){
                                                tArray = $.merge(dataAsArray,deldata);
                                                console.log(tArray);
                                                $('#actualTbl').show();
            ...
            $(function () {
                ...
                $('#sendServer').click(function () {
                    ...
                    $('#sendServer').addClass('noClick disabled').text('Creating....Please wait')
                    var gp_name = $('#grp_name').val();

                    if (gp_name === "") {
                        toastr.error('please select group name');
                        $('#sendServer').removeClass('noClick disabled').text('Create Group')
                        return false;
                    } else {
                        var xxtable = $('#fntbl').DataTable();
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                grpName: group_name,
                                grpMrks: grp_mrks,
                                content: JSON.stringify(dataAsArray),
                            },
                            success: function (data) {
                            ...
                            error: function () {
                                toastr.error('Cant create group.Please try again later');
                                $('#sendServer').removeClass('noClick disabled').text('Create Group')
                            }
                        });
                    }
