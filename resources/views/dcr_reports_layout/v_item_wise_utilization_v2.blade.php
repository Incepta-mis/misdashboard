...
@section('title','Item Wise Utilization Report')
@section('styles')
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-2 col-sm-2 col-xs-6">
                                <select name="terr_id" id="terr_id" class="form-control input-sm">
                                    <!-- <option value="All">All</option> -->
                                    @foreach($terr_id as $ttid)
                                        <option value="{{$ttid->terr_id}}">{{$ttid->terr_id}}</option>
                                    @endforeach
                            ...
                            <div class="col-md-2 col-sm-2 col-xs-6">
                                <select name="item_name" id="item_name" class="form-control input-sm">
                                    <option value="All">All</option>
                                    @forelse($item_name as $ttnam)
                                        <option value="{{$ttnam->item_name}}">{{$ttnam->item_name}}</option>
                                    @empty
                                        <option value="d">No data found</option>
                                    @endforelse
                            ...
                            <label class="control-label col-md-2 col-sm-2 col-xs-6 input-sm" for="date_from" style="width: 12%;">
                                <b>DCR Month:</b></label>

                            <div class="col-md-2 col-sm-2 col-xs-6">
                                <select name="dcr_month" id="dcr_month" class="form-control input-sm">
                                    @foreach($dcr_month as $dcr_mon)
                                        <option value="{{$dcr_mon->mnth}}">{{$dcr_mon->mnth}}</option>
                                    @endforeach
            ...
            <div class="col-sm-12 col-md-12">
                <section class="panel" id="data_table">
                    <div class="panel-body table-responsive">
                        <table id="item_uti_data" class="table table-condensed table-striped table-bordered"
                               width="100%">
...
@section('scripts')
    ...
    {{Html::script('public/site_resource/js/dcr_scripts/iwur_script.js')}}

    <script>
        servloc = "{{url('dcrep/resp_item_utilize_rep')}}";servloc_t = "{{url('dcrep/resp_item_utilize_terr_id')}}";
        servloc_name = "{{url('dcrep/resp_item_utilize_items')}}";eid = "{{Auth::user()->user_id}}";desig = "{{Auth::user()->desig}}";
    </script>
@endsection
