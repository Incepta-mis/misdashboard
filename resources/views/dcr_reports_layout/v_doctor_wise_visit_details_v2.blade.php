...
@section('title','Doctor Wise Visit Details')
@section('styles')
                            ...
                            <div class="col-md-2 col-sm-2 col-xs-6">
                                <select name="terr_id" id="terr_id" class="form-control input-sm">
                                    @foreach($terrs as $terr)
                                        <option value="{{$terr->terr_id}}">{{$terr->terr_id}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <label class="control-label col-md-2 col-sm-2 col-xs-6 input-sm" style="width: 12%;"
            ...
            <div class="col-sm-12 col-md-12">
                <section class="panel" id="data_table">
                    <div class="panel-body table-responsive">
                        <table id="dwvd_tab" class=" table-fixed-header table table-fixed table-condensed table-striped table-bordered"
                               width="100%">
...
@section('scripts')
    ...
    {{Html::script('public/site_resource/js/dcr_scripts/dwvd_script2.js')}}


    <script>
        servloc = "{{url('dcrep/resp_dwvd_rep')}}";
        servloc_t = "{{url('dcrep/resp_vdtd_id')}}";
        eid = "{{Auth::user()->user_id}}";desig = "{{Auth::user()->desig}}";
        loc_rterr = "{{url('dcrep/resp_terr_list_dwvd')}}";
    </script>
@endsection
