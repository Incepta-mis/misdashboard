...
@section('title','Raw_Pack/ Capex Committed Report')
@section('styles')
            ...
            <section class="panel panel-default">
                <div class="panel-heading">
                    <label class="text-default">
                        Raw_Pack/ Capex Committed Report
                    </label>
                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#btn_submit').on('click', function (e) {
                ...
                if(prod_type === "" && acc_date === "" && payment_status === "" && due_date_act_from === "" && due_date_act_to === ""){
                    $("#loader").hide();
                    toastr.error("Please input at least one data!");
                }else {
                    if(prod_type === ""){
                        $("#loader").hide();
                        toastr.error("You must select a product type");
                    }else if (due_date_act_from !== "" && due_date_act_to === "") {
                        $("#loader").hide();
                        toastr.error("You must choose a date range");
                    } else if (due_date_act_from === "" &&
                        due_date_act_to !== "") {
                        $("#loader").hide();
                        toastr.error("You must choose a date range");
                    } else if (due_date_act_from !== "" && due_date_act_to !== "" && (checkdateFrom > checkdateTo)) {
                        $("#loader").hide();
                        toastr.error("Second date must be greater than the first date!");
                    } else {
                        $.ajax({
                            type: 'post',
                            url: '{{ url('import_management/getCommittedReport') }}',
                            data:{'prod_type':prod_type, 'acc_date':acc_date, 'payment_status':payment_status,
                                    ...
                                    for (var j = 0; j < data.result.length; j++) {
                                        ...
                                        for (var z = 0; z < dataResult.length; z++) {
                                            if (dataResult[z]['result'].length > 0) {
                                                var acceptance_value_in_bdt = 0;
                                                for (var y = 0; y < dataResult[z]['result'].length; y++) {
                                                    if(dataResult[z]['result'][y]['acceptance_value_in_bdt'] != null){
                                                        acceptance_value_in_bdt = parseFloat(acceptance_value_in_bdt) + parseFloat(dataResult[z]['result'][y]['acceptance_value_in_bdt']);
                                                    }
                                                }
                                                totalAccptValueInBDT = parseFloat(totalAccptValueInBDT) + parseFloat(acceptance_value_in_bdt);
                                                tbodyhtml += "<td>" + acceptance_value_in_bdt + "</td>";
                                            } else {
                                                tbodyhtml += "<td></td>";
                                            }
                                        }
        ...
        function ExportToExcel(type, fn, dl) {
            ...
            return dl ?
                ...
                XLSX.writeFile(wb, fn || ('committed_capex_rawpack.' + (type || 'xlsx')));
        }
    </script>
    @endsection
