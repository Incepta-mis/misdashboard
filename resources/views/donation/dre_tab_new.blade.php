<table class="table table-condensed table-striped table-bordered" id="dre" ng-cloak="">
    ...
    <tbody>
    <tr ng-repeat="td in dataArr" ng-if="dataArr.length > 0" id="row_@{{ $index }}">
        <td>
        ...
        <td style="white-space: nowrap;">
            <input type="text" name="doc_id" class="doc_id form-control"
                   ...
                   maxlength="6" size="6"
                   ng-enter="fetchDoctor(td.doc_id,$index,td.pay_mode)" readonly>
        </td>
        ...
        <!-- <td>
            <select name="exp_type" class="exp_type form-control"  
            ng-model="td.exp_type" ng-change="fetch_gbr($index,td.exp_type)">
            <option ng-repeat="dtype in expType"
                        value="@{{dtype.type_name}}">
                    @{{ dtype.type_name }}
                </option> 
                <!-- ng-options="item.type_name for item in expType" -->
                <!-- <option  value="@{{td.exp_type}}">@{{td.exp_type}}</option> -->
                <!-- <option ng-repeat="dtype in expType"
        ...
        <td>
            <select name="gbr_name"  class="gbr_name form-control" ng-model="td.gbr_name" ng-change="reset_pay_mode($index)">
                <option value="@{{ s.cost_center_id }}|@{{ s.cost_center_name }}|@{{ s.sub_cost_center_id }}" 
                ng-repeat="s in td.gbrList">@{{ s.cost_center_name }}</option>
            </select>
        </td>
        <td>
            <select name="frequency" ng-disabled="td.freqRonly" class="frequency form-control" ng-model="td.frequency" ng-change = "hideB()">
            <!-- ng-change="checkDoctor($index)" -->
        ...
        <td>
            <select name="pay_mode" ng-disabled="td.pmodeRonly" class="pay_mode form-control" ng-model="td.pay_mode"
                    ng-change="checkBEFTNmasterData($index,td.pay_mode,td.proposed_amt)">
        ...
        <td>
            <input type="text" name="proposed_amt" class="proposed_amt form-control"
                   ...
                   autocomplete="off" maxlength="8" size="8" ng-change="checkval($index,td.proposed_amt)"
                   ng-enter="addAmount($index,td.proposed_amt,td.pay_mode)">
        </td>
    </tr>
